...
def criar_card(parent, numero, texto):
    card = tk.Frame(parent, bg="white", bd=1, relief="solid", width=180, height=100)
    card.pack_propagate(False)
    card.pack(side="left", padx=10)
    
    tk.Label(card, text=numero, font=("Segoe UI", 20, "bold"), bg="white").pack(pady=(15, 5))
    tk.Label(card, text=texto, font=("Segoe UI", 10), bg="white").pack()

def criar_botao_dashboard(parent, texto):
    return tk.Button(parent, text=texto, width=18, height=3, bg=COR_BOTAO, relief="solid", bd=1)
...
def abrir_dashboard():
    limpar_conteudo()
    # Código para reconstruir o Dashboard virá aqui
...
for texto, comando in menus.items():
    botao = tk.Button(frame_menu, text=texto, anchor="w", relief="flat", bg="#F5F7F7", padx=15, command=comando)
...
# DASHBOARD: CABEÇALHO
cabecalho = tk.Frame(frame_conteudo, bg="white", height=80)
cabecalho.pack(fill="x")

titulo_dashboard = tk.Label(cabecalho, text="Dashboard", font=FONTE_TITULO, bg="white")
...
titulo_acesso = tk.Label(frame_conteudo, text="Acesso Rápido", font=FONTE_SUBTITULO, bg="white")
titulo_acesso.pack(anchor="w", padx=30, pady=(20, 10))

frame_acesso = tk.Frame(frame_conteudo, bg="white")
frame_acesso.pack(anchor="w", padx=30)

linha1 = tk.Frame(frame_acesso, bg="white")
linha1.pack(pady=5)

linha2 = tk.Frame(frame_acesso, bg="white")
linha2.pack(pady=5)
...
titulo_atividade = tk.Label(frame_conteudo, text="Atividade Recente", font=("Segoe UI", 14, "bold"), bg="white")
titulo_atividade.pack(anchor="w", padx=30, pady=(25, 10))

frame_atividade = tk.Frame(frame_conteudo, bg="white", bd=1, relief="solid")
...
for atividade in atividades:
    lbl = tk.Label(frame_atividade, text=atividade, bg="white", anchor="w", font=FONTE_NORMAL)
